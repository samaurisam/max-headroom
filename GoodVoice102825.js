...
const VoiceInterface = ({
  ...
  const Captions = ({ messages }) => {
    ...
    const latestAgentMessage =
      agentMessages.length > 0 ? agentMessages[agentMessages.length - 1] : null;
  ...
  const startAudioVisualizer = (stream) => {
    ...
    const visualize = () => {
      ...
      if (average > VAD_THRESHOLD) {
        ...
        vadState === "speaking" &&
        now - lastSpeechRef.current < VAD_HOLD_MS
      ) {
    ...
    try {
      ...
      const conv = await Conversation.startSession({
        ...
        onModeChange: (m) => {
          ...
          if (onSpeechIntensity) onSpeechIntensity(isSpeaking ? 1 : 0);
        },
        onEndOfUtterance: () => {
          console.log("Agent finished speaking → resetting glitch");
          onGlitchIntensity(0);
          if (onSpeechIntensity) onSpeechIntensity(0);
        },
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "12px" }}>
          <button
            ...
            style={{
              ...
              border: "none",
              borderRadius: "6px",
              background: isConnecting ? "#64748b" : "#3b82f6",
              color: "#fff",
              cursor: "pointer",
            }}
          >
